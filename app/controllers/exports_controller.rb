class ExportsController < ApplicationController
  ...
  def generate_legislation_csv(legislation)
    ...
    CSV.generate do |csv|
      csv << ["Field", "Value"]
      csv << ["Title", legislation.title]
      csv << ["FRBR URI", legislation.frbr_uri]
      csv << ["CELEX", legislation.celex_number]
      csv << ["ELI URI", legislation.eli_uri]
      csv << ["Jurisdiction", legislation.jurisdiction.name]
      csv << ["Type", legislation.legislation_type]
      ...
      legislation.legislation_versions.each do |v|
        csv << [v.version_uri, v.language, v.valid_from, v.valid_to, v.version_type]
      end
    end
  end
  ...
  def generate_search_csv(results)
    ...
    CSV.generate do |csv|
      ...
      results.each do |leg|
        csv << [leg.title, leg.jurisdiction.code.upcase, leg.legislation_type, leg.year, leg.status, leg.celex_number, leg.frbr_uri]
      end
    end
  end
end
