class DocumentNode < ApplicationRecord
  ...
  scope :descendants_of, ->(path) {
    where("tree_path <@ ?::ltree AND tree_path != ?::ltree", path.to_s, path.to_s)
  }
  scope :ancestors_of, ->(path) {
    where("tree_path @> ?::ltree AND tree_path != ?::ltree", path.to_s, path.to_s)
  }
  scope :subtree_of, ->(path) { where("tree_path <@ ?::ltree", path.to_s) }
  ...
  validates :element_type, inclusion: { in: HIERARCHICAL_TYPES }, allow_nil: true
end
